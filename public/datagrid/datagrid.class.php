...
##  Last changed:  23.01.2017 13:22                                            #
##                                                                             #
##  Additional modules (embedded):                                             #
##  -- openWYSIWYG v1.0.1 (WYSIWYG editor)              http://openWebWare.com #
##  -- JS AFV v3.3.3 (JS Auto From Validator)                 http://apphp.com #
...
class DataGrid
{
    ...
    // Data Members
    //==========================================================================
    static private $dgVersion = '8.2.7';
        ...
        if(($auto_columns == true) || ($auto_columns == 'true')){
            ...
            if($this->dbType == 'PDO'){
                ...
                if($this->CheckIsError($dSet)){
                    ...
                    if($this->dgGetDbDriverType() == 'mysql') $sql_result = mysqli_query($this->dbHandler, ' DESCRIBE '.$this->tblName.' ');
    ...
    public function Bind($show = true){
        ...
        $req_mode       = $this->GetVariableAlpha('mode');
        $req_mode_parameters = $this->GetVariableAlpha('mode_parameters');
        ...
        if($this->debug){
            ...
            echo '<br /><center><label class="default_dg_label">DataGrid v'.self::$dgVersion.' | Extension: '.$this->dbType.' | Database: '.$this->dbDriver.' | Total running time: '.round((float)$this->endTime - (float)$this->startTime, 6).' sec.</label></center><br />';
        }

        echo $this->nl.'<!-- END This code was generated by datagrid.class.php v'.self::$dgVersion.' (http://www.apphp.com/php-datagrid/index.php) END -->'.$this->nl;
            ...
            for($c_sorted = $this->colLower; $c_sorted < $this->sortedColumnsSize; $c_sorted++){
                ...
                if(($this->mode === 'view') && ($this->CanViewField($c_field_name))){
                    ...
                    if($fp_summarize){
                        ...
                        else if($fp_summarize_function == 'SUM' || $fp_summarize_function == 'AVG'){ $this->arrSummarizeColumns[$c_field_name]['sum'] += $curr_value; }
                        ...
                        if($count > 0){
                            ...
                            if($this->multirowViewType != 'dropdownlist'){
                                echo '<td><label class="'.$this->cssClass.'_dg_label" style="padding-left:10px;padding-right:0px;"><i>'.$this->lang['with_selected'].':</i></label></td>
                                      <td align="'.(($this->direction == 'ltr') ? 'left' : 'right').'" '.(($this->browserName != 'MSIE') ? ' style="padding-bottom:3px;"' : '').'>';
                            }else{
                                echo '<td>&nbsp;';
    ...
    protected function DisplayDataSent(){
        if($this->debug){
            ...
            if(isset($_POST) && is_array($_POST)){
                foreach($_POST as $key => $val) echo '<br>['.htmlentities($key).'] => '.(!is_array($val) ? strip_tags($val) : $val);
            }
            print_r('</span><br /><br />');
            print_r('<span class="'.$this->cssClass.'_dg_ok_message no_print"><b><u>GET</u></b>: ');
            if(isset($_GET) && is_array($_GET)){
                foreach($_GET as $key => $val) echo '<br>['.htmlentities($key).'] => '.(!is_array($val) ? strip_tags($val) : $val);
            }
            print_r('</span><br /><br />');
    ...
    protected function AddRow(){
        ...
        if(!$unique_field_found){
            ...
            $ind = 0;
            $max = count($_POST);
            if($this->dgGetDbDriverType() == 'oci8'){ $sql_fields .= strtoupper($this->primaryKey).', '; }
            if($this->dgGetDbDriverType() == 'oci8'){ $sql_values .= '(SELECT '.$this->tblName.'.'.$this->primaryKey.' FROM '.$this->tblName.'), '; }
            foreach($_POST as $fldName => $fldValue){
                ...
                if(!$this->IsExceptedField($fldName, 'add') && !$this->IsValidator($fldName)){
                    ...
                    if($this->IsDate($fldName)){
                        ...
                        if(trim($fldValue) != ''){
                            if($this->convertOutput) $fldValue = hebrev(mb_convert_encoding($fldValue, $this->convertFrom, $this->convertTo));
                            if($this->dgGetDbDriverType() == 'oci8') $sql_values .= (int)$fldValue;
                            ...
                            if($this->GetFieldProperty($field_name, 'host') == 'remote'){
                                ...
                                if($file == true){
                                    ...
                                    if($field_type == 'image'){
                                        ...
                                        $ret_file .= $this->nbsp.'<img src="'.$this->directory.'images/mimetypes/'.Helper::GetFileMimetype($field_value).'.png" alt="'.htmlspecialchars(Helper::GetFileMimetype($field_value)).'" style="margin:0 0 -3px 5px;width:16px;height:16px;" />'.$this->nbsp;
                                    }
                    ...
                    switch($field_type){
                        ...
                        case 'file':
                            ...
                            if($fp_save_as == 'blob'){
                                ...
                                $output .= '<img src="'.$this->directory.'images/mimetypes/'.Helper::GetFileMimetype($field_value).'.png" alt="'.htmlspecialchars(Helper::GetFileMimetype($field_value)).'" style="margin:0 5px -3px 5px;width:16px;height:16px;" />';
...
';
    ...
    protected function DrawCalendarButton($field_name, $field_type, $datetime_format='Y-m-d', $field_value='', $field_width='', $field_maxlength='', $on_js_event='', $readonly=false, $fp_calendar_type = 'popup', $multirow_postfix = ''){
        ...
        $show_time = 'false';
        $show_date = true;
        if(!Helper::IsDateTime($field_value)) $field_value = '';
    ...
    private function SetCommonJavaScript(){
        ...
        echo $this->nl.'<!-- START This code was generated by datagrid.class.php v'.self::$dgVersion.' (http://www.apphp.com/php-datagrid/index.php) START -->';
