			        ...
			        <form id="rowForm" name="rowForm" class="form-horizontal">
			           <input type="hidden" name="IdHor" id="IdHor">                    
                        <div class="form-group">
			                <label for="hor_desc" class="col-sm-2 control-label">Horario</label>
			                <div class="col-sm-12">
			                    <input type="text" class="form-control" id="hor_desc" name="hor_desc" placeholder="Horario" value="" maxlength="12" required="">
			                </div>
                        </div>	
...
<script src="../../js/ui-horarios.js"></script>
	...
	$(function() {
		...
		$('#grdDatos').DataTable({
			...
			$('body').on('click', '.edit-row', function () {
				var IdHor = $(this).data('id');

				$.get("{{ route('horarios.index') }}" +'/' + IdHor +'/edit', function (data) {
					$('#rowCrudModal').html("Editar registro");
					$('#btn-save').val("edit-row");
                    $('#ajax-crud-modal').modal('show');
                    $('#cboEstatus option[value=' + data.hor_idest + ']').attr('selected', 'selected');
					$('#IdHor').val(data.IdHor);
                    $('#hor_desc').val(data.hor_desc);					
				})
			});
		});
		if ($("#rowForm").length > 0) {
